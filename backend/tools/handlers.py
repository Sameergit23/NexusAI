"""Tool handlers — the bridge between Claude's tool calls and the agents.

Every handler:
  - takes a single `args` dict (Claude's tool input + an injected `run_id`),
  - reads the canonical data from the shared run context (so it is robust even
    if Claude's relayed JSON is imperfect),
...
async def handle_plan_tasks(args: dict) -> dict:
    ...
    db.ctx_set(run_id, "zones", result["zones"])
    return result
...
async def handle_generate_report(args: dict) -> dict:
    run_id = args["run_id"]
    naive_km = db.ctx_get(run_id, "naive_km")
    if naive_km is None:
        naive_km = args.get("naive_km", 0)
    optimised_km = db.ctx_get(run_id, "optimised_km")
    if optimised_km is None:
        optimised_km = args.get("optimised_km", 0)
    deliveries = db.ctx_get(run_id, "deliveries") or []
    total = len(deliveries) or args.get("deliveries_total", 0)
    on_time = args.get("deliveries_on_time", total)  # demo: assume all on time
